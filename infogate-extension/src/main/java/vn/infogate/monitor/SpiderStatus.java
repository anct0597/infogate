package vn.infogate.monitor;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import vn.infogate.Spider;
import vn.infogate.scheduler.MonitoredScheduler;
...
public class SpiderStatus implements SpiderStatusMXBean {
    ...
    public int getLeftPageCount() {
        if (spider.getScheduler() instanceof MonitoredScheduler) {
            return ((MonitoredScheduler) spider.getScheduler()).getLeftRequestsCount(spider);
        }
        logger.warn("Get leftPageCount fail, try to use a Scheduler implement MonitoredScheduler for monitor count!");
        return -1;
    }

    public int getTotalPageCount() {
        if (spider.getScheduler() instanceof MonitoredScheduler) {
            return ((MonitoredScheduler) spider.getScheduler()).getTotalRequestsCount(spider);
        }
        logger.warn("Get totalPageCount fail, try to use a Scheduler implement MonitoredScheduler for monitor count!");
        return -1;
    }
